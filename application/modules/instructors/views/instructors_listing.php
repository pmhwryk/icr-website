				...
				<div class="m-portlet__head-caption">
					<div class="m-portlet__head-title">
						<h3 class="m-portlet__head-text">
							<i class="m-menu__link-icon flaticon-users"></i> Instructors
						</h3>
					</div>
				</div>
				<div class="m-portlet__head-tools">
					<ul class="m-portlet__nav">
						<li class="m-portlet__nav-item">
							<a href="<?= ADMIN_BASE_URL ?>instructors/create" class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
								<span>
					...
					<tbody>
						...
						if (isset($instructor)) {
							foreach ($instructor as $row) :
								...
								$edit_url = ADMIN_BASE_URL . 'instructors/create/' . $instructor_id;
						?>
								<tr>
									<td>
										<?php echo $i; ?>
									...
									<td>
										<?= (isset($row['designation']) && !empty($row['designation']) ? $row['designation'] : "") ?>
									</td>
									<td nowrap="">
										<span class="dropdown">
	...
	$(document).off('click', '.delete_record').on('click', '.delete_record', function(e) {
		...
		swal({
			...
			if (e.value) {
				$.ajax({
					type: 'POST',
					url: "<?= ADMIN_BASE_URL ?>instructors/delete",
					data: {
						'id': id,
