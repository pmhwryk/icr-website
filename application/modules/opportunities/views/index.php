				...
				<div class="m-portlet__head-caption">
					<div class="m-portlet__head-title">
						<h3 class="m-portlet__head-text">
							<i class="m-menu__link-icon flaticon-edit"></i> Opportunities
						</h3>
					</div>
				</div>
				<div class="m-portlet__head-tools">
					<ul class="m-portlet__nav">
						<li class="m-portlet__nav-item">
							<a href="<?=ADMIN_BASE_URL?>opportunities/create" class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
								<span>
					...
					<tbody>
                        ...
                        if (isset($query)) {
                        	foreach ($query->result() as $row):
                        		$i++;   
                        		$edit_url = ADMIN_BASE_URL . 'opportunities/create/' . $row->id;
                        ?>
						<tr>
							<td>
								<?php echo $i;?>
...
<script type="application/javascript">
	$(document).off('click', '.delete_record').on('click', '.delete_record', function(e){
        ...
        e.preventDefault();
		swal({
			title : "Are you sure to delete the selected opportunities?",
			text : "You will not be able to recover this opportunities!",
			type : "warning",
			...
			if(e.value) {
				$.ajax({
					type: 'POST',
					url: "<?= ADMIN_BASE_URL?>opportunities/delete",
					data: {'id': id},
					async: false,
					success: function() {
						swal("Deleted!", "Opportunities has been deleted.", "success");
						location.reload();
					}
				});
				swal("Deleted!", "Opportunities has been deleted.", "success");
			...
			if("cancel" === e.dismiss)
				swal("Cancelled", "Opportunities is safe :)", "error");
		});
    });
	...
	$(document).on("click", ".view_details", function(event){
		...
		$.ajax({
			type: 'POST',
			url: "<?=ADMIN_BASE_URL?>opportunities/detail",
			data: {'id': id},
			async: false,
    ...
    $(document).off("click",".action_publish").on("click",".action_publish", function(event) {
		...
		$.ajax({
		type: 'POST',
			url: "<?= ADMIN_BASE_URL ?>opportunities/change_status",
