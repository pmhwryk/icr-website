...
class Enrollment_request extends MX_Controller
{
	...
	function index()
	{
		$data['query'] = Modules::run('api/get_specific_table_with_pagination_where_groupby', array("approved_enrollment"=>0), 'id desc', 'id', 'joining_requests', '*', '', '0', '', '', '')->result_array();
	...
	function detail()
	{
		...
		if (!empty($update_id) && is_numeric($update_id) && $update_id > 0)
			$data['query'] = Modules::run('api/get_specific_table_with_pagination_where_groupby', array('id' => $update_id), 'id desc', 'id', 'joining_requests', '*', '', '0', '', '', '')->row_array();
	...
	function get_receiver_name()
	{
		$update_id = $this->input->post('id');
		if (!empty($update_id) && is_numeric($update_id) && $update_id > 0)
			$data['installments'] = $this->get_payment_details_with_students(array('installments.id' => $update_id, 'installments.outlet_id' => DEFAULT_OUTLET), 'installments.id desc', 'installments.id', '', '0', '', '', '', '0')->row_array();
	...
	function check_student_fee()
	{
		...
		$data['fee'] = Modules::run('api/get_specific_table_with_pagination_where_groupby', array('id' => $student_id, 'outlet_id' => DEFAULT_OUTLET), 'id', 'id', 'students', 'net_fee', '1', '1', '', '', '')->row_array();
	...
	function approve_request()
	{
		$id = $this->input->post('id');
			$data['approved_enrollment'] = 1;
		Modules::run('api/update_specific_table', array('id' => $id), $data, 'joining_requests');
		echo $$data['approved_enrollment'];
	}
	...
	function delete()
	{
		...
		if (isset($id) && !empty($id) && $id > 0) {
			...
			Modules::run('api/delete_from_specific_table', array('id' => $id), 'joining_requests');
		}
	}
}
