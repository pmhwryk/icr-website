				...
				<div class="m-portlet__head-caption">
					<div class="m-portlet__head-title">
						<h3 class="m-portlet__head-text">
							<i class="m-menu__link-icon flaticon-edit"></i> Package Type
						</h3>
					</div>
				</div>
				<div class="m-portlet__head-tools">
					<ul class="m-portlet__nav">
						<li class="m-portlet__nav-item">
							<a href="<?=ADMIN_BASE_URL?>pkg_type/create" class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
								<span>
					...
					<tbody>
                        ...
                        if (isset($query)) {
                        	foreach ($query->result() as $row):
                        		$i++; 
                        		$edit_url = ADMIN_BASE_URL . 'pkg_type/create/' . $row->id;
                        		$pkg_name= Modules::run('api/get_specific_table_with_pagination_where_groupby',array('id'=> $row->pkg_id), 'id','id','pkg','name','1','0','','','')->row_array();
						...
						<tr>
                            ...
                            <td>
								<?=(isset($pkg_name) && !empty($pkg_name['name'])? $pkg_name['name'] :'')?>
                            </td>
							<td nowrap="">
								<span class="dropdown">
...
<script type="application/javascript">
	$(document).off('click', '.delete_record').on('click', '.delete_record', function(e){
        ...
        e.preventDefault();
		swal({
			title : "Are you sure to delete the selected Pricing?",
			text : "Pricing will not recover once deleted!",
			type : "warning",
			...
			if(e.value) {
				$.ajax({
					type: 'POST',
					url: "<?= ADMIN_BASE_URL?>pkg_type/delete",
					data: {'id': id},
					async: false,
					success: function() {
						swal("Deleted!", "Pricing has been deleted.", "success");
						location.reload();
					}
				});
				swal("Deleted!", "Pricing has been deleted.", "success");
			...
			if("cancel" === e.dismiss)
				swal("Cancelled", "Pricing is safe :)", "error");
		});
    });
	$(document).on("click", ".view_details", function(event){
		...
		$.ajax({
			type: 'POST',
			url: "<?=ADMIN_BASE_URL?>pkg_type/detail",
			data: {'id': id},
			async: false,
		...
		/*$(document).on("click", ".edit_details", function(event){
			...
			$.ajax({
					type:'POST',
					url:"<?=ADMIN_BASE_URL?>pkg_type/create/".id,
					data:{'id':id},
					async:false,
