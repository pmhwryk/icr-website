				...
				<div class="m-portlet__head-caption">
					<div class="m-portlet__head-title">
						<h3 class="m-portlet__head-text">
							<i class="m-menu__link-icon flaticon-edit"></i> Faqs
						</h3>
					</div>
				</div>
				<div class="m-portlet__head-tools">
					<ul class="m-portlet__nav">
						<li class="m-portlet__nav-item">
							<a href="<?=ADMIN_BASE_URL?>faqs/create" class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
								<span>
					...
					<tbody>
                        ...
                        if (isset($query)) {
                        	foreach ($query->result() as $row):
                        		$i++;   
                        		$edit_url = ADMIN_BASE_URL . 'faqs/create/' . $row->id;
                        ?>
						<tr>
							<td>
								<?php echo $i;?>
...
<script type="application/javascript">
	$(document).off('click', '.delete_record').on('click', '.delete_record', function(e){
        ...
        e.preventDefault();
		swal({
			title : "Are you sure to delete the selected Faqs?",
			...
			if(e.value) {
				$.ajax({
					type: 'POST',
					url: "<?= ADMIN_BASE_URL?>faqs/delete",
					data: {'id': id},
					async: false,
					success: function() {
	...
	$(document).on("click", ".view_details", function(event){
		...
		$.ajax({
			type: 'POST',
			url: "<?=ADMIN_BASE_URL?>faqs/detail",
			data: {'id': id},
			async: false,
    ...
    $(document).off("click",".action_publish").on("click",".action_publish", function(event) {
		...
		$.ajax({
		type: 'POST',
			url: "<?= ADMIN_BASE_URL ?>faqs/change_status",
