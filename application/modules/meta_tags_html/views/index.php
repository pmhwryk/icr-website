				...
				<div class="m-portlet__head-caption">
					<div class="m-portlet__head-title">
						<h3 class="m-portlet__head-text">
							<i class="m-menu__link-icon flaticon-edit"></i> Meta Tags HTML
						</h3>
					</div>
				</div>
				<div class="m-portlet__head-tools">
					<ul class="m-portlet__nav">
						<li class="m-portlet__nav-item">
							<a href="<?=ADMIN_BASE_URL?>meta_tags_html/create" class="btn btn-primary m-btn m-btn--pill m-btn--custom m-btn--icon m-btn--air">
								<span>
					...
					<tbody>
                        ...
                        if (isset($query)) {
                        	foreach ($query->result() as $key => $row):   
                        		$edit_url = ADMIN_BASE_URL . 'meta_tags_html/create/' . $row->id;
                        ?>
						<tr>
...
<script type="application/javascript">
	$(document).off('click', '.delete_record').on('click', '.delete_record', function(e){
        ...
        e.preventDefault();
		swal({
			title : "Are you sure to delete the selected meta_tags_html?",
			text : "You will not be able to recover this meta_tags_html!",
			type : "warning",
			...
			if(e.value) {
				$.ajax({
					type: 'POST',
					url: "<?= ADMIN_BASE_URL?>meta_tags_html/delete",
					data: {'id': id},
					async: false,
					success: function() {
						swal("Deleted!", "meta_tags_html has been deleted.", "success");
						location.reload();
					}
				});
				swal("Deleted!", "meta_tags_html has been deleted.", "success");
			...
			if("cancel" === e.dismiss)
				swal("Cancelled", "meta_tags_html is safe :)", "error");
		});
    });
	...
	$(document).on("click", ".view_details", function(event){
		...
		$.ajax({
			type: 'POST',
			url: "<?=ADMIN_BASE_URL?>meta_tags_html/detail",
			data: {'id': id},
			async: false,
	...
	$(document).on("click", ".action_top_page", function(event) {
		...
		$.ajax({
			type: 'POST',
			url: "<?=ADMIN_BASE_URL ?>meta_tags_html/set_as_homepage",
    ...
    $(document).off("click",".action_publish").on("click",".action_publish", function(event) {
		...
		$.ajax({
		type: 'POST',
			url: "<?= ADMIN_BASE_URL ?>meta_tags_html/change_status",
			data: {'id': id, 'status': status},
			async: false,
			success: function(result) {
